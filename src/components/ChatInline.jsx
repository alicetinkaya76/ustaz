    ...
    <div className="card">
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-2 ${msg.role === "user" ? "justify-end" : ""}`}>
              ...
              <div className={`max-w-[85%] rounded-xl px-3 py-2 text-sm leading-relaxed ${
                msg.role === "user" ? "bg-ustaz-gold/10 text-ustaz-turkish" : msg.error ? "bg-red-500/10 text-red-300" : "bg-white/5 text-ustaz-turkish/80"
              }`}>
        ...
        <div className="mb-3 flex flex-wrap gap-1.5">
          {QUICK_QUESTIONS.map((q, i) => (
            <button key={i} onClick={() => handleSend(q)}
              className="rounded-full border border-white/5 bg-white/[0.02] px-3 py-1.5 text-[11px] text-ustaz-turkish/40 transition hover:border-ustaz-gold/20 hover:bg-white/5 hover:text-ustaz-turkish/60">
              {q}
            </button>
          ))}
        </div>
      ...
      <div className="flex gap-2">
        <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown}
          ...
          className="flex-1 rounded-lg border border-white/5 bg-ustaz-bg/50 px-3 py-2.5 text-sm text-ustaz-turkish placeholder:text-ustaz-turkish/20 focus:border-ustaz-gold/30 focus:outline-none disabled:opacity-40" />
